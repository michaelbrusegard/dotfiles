			...
			vim.api.nvim_create_autocmd("LspAttach", {
				callback = function(event)
					...
					vim.keymap.set(
						"n",
						"gd",
						"<cmd>lua require('util.navigation').pick('lsp_definitions', { jump_to_single_result = true, ignore_current_line = true })<cr>",
						{ desc = "Goto Definition", buffer = event.buf }
					)
					vim.keymap.set(
						"n",
						"gr",
						"<cmd>lua require('util.navigation').pick('lsp_references', { jump_to_single_result = true, ignore_current_line = true })<cr>",
						{ desc = "References", buffer = event.buf }
					)
					vim.keymap.set(
						"n",
						"gI",
						"<cmd>lua require('util.navigation').pick('lsp_implementations', { jump_to_single_result = true, ignore_current_line = true })<cr>",
						{ desc = "Goto Implementation", buffer = event.buf }
					)
					vim.keymap.set(
						"n",
						"gy",
						"<cmd>lua require('util.navigation').pick('lsp_typedefs', { jump_to_single_result = true, ignore_current_line = true })<cr>",
